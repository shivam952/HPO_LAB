...
class Hyperband(HPOAlgorithm):
    ...
    Runs a portfolio of s_max+1 Successive Halving brackets in parallel, each
    with a different trade-off between number of initial configurations (n_s)
    and starting fidelity (r_s). Brackets are iterated round-robin; when all
    are exhausted the entire portfolio is restarted with fresh configurations.

    The key advantage over standalone SH: the most exploitative bracket (s=0)
    starts directly at r_max, guaranteeing that at least one bracket always
    evaluates configurations at full fidelity, which standalone SH cannot do
    when r_max is not a power of eta times r_min.

    Args:
        cs:           ConfigurationSpace defining the hyperparameter search space.
        total_budget: Total fidelity budget across all evaluations.
        min_budget:   Minimum fidelity per evaluation (r_min).
        max_budget:   Maximum fidelity per evaluation (r_max).
        eta:          Halving/promotion rate (default 3).
