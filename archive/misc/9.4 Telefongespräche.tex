...
% metadata
\title[Assignment 1]{Empirical comparison \newline of HPO methods on YAHPO benchmarks}
...
\begin{frame}
  ...
  \begin{itemize}
      \item Maybe it would be smarter to focus on what we had to design to show the difference in  design decisions?
      \item Because everyone will say the same thing in this part...
  \end{itemize}
...
\begin{frame}
  \frametitle{Experimental design choices}

  \begin{itemize}[<+->]
    \item Same budget across all methods $\leadsto$ fair comparison
    \item Total budget = max\_budget $\times$ 50
    \item 5 $\strike{non}$ random seeds per method
    \item Two benchmarks
    \begin{itemize}
      \item NB301 (CIFAR10)
      \item RBV2 XGBoost
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item Uniform sampling over config space (describe/show the implementation?)
    \item No learning $\leadsto$ Constant max budget evaluation
    \item Lower bound baseline
  \end{itemize}
...
\begin{frame}
  \frametitle{Successive Halving}

  \begin{itemize}[<+->]
    \item $\eta$ = 2 fixed
    \item Parameters derived from budget constraint
    \item n\_rounds = math.floor(math.log(max\_budget / min\_budget, eta)) + 1
    \item n\_candidates = max(1, math.floor(total\_budget / (min\_budget * self.n\_rounds)))
    \item pruning $\leadsto$ top $\frac{1}{\eta}$ by validation score
    \item budget escalation capped at max\_budget
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item $\eta$ = 2 fixed
    \item s\_max = math.floor(math.log(max\_budget / min\_budget, eta))
    \item Do we need more formulae?
    \item Did you reuse the SH logic? Me not, because it would require more abstractions and the implementation would take longer
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item Categorical encoding strategy (describe/show the implementation?)
    \item warmup phase = 10 iterations reusing Random Search
    \item random candidate pool $\leadsto$ 1000 samples
    \item Why? Idk, why not? :D
    \item Constant max budget evaluation
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item Our mistakes will make our presentation different \& unique :)
    \item Fidelity parameter drop vs inject back
    \item Budget vs fidelity synchronization bugs
    \item Successive Halving \& HyperBand queue lifecycle
    \item anything else?
...
\begin{frame}[fragile]
  \frametitle{Results \& Key findings}

  Short Description
  \twocol{
    \begin{itemize}
      \item Random Search is one of the most successful models...
      \item ...or maybe we're just lucky :D
      \item Hyperband $>$ Successive Halving
      \item Results vary across benchmarks (why? our assumtions)
    \end{itemize}
  }{
    \vskip-15pt
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item No single best optimizer across all settings (classic conclusion)
    \item Implementation choices strongly affect performance (if we will show results for $\eta = 3+$, warmup duration, or random candidate pool size)
  \end{itemize}
