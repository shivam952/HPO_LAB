...
from bohb import BOHB_Round, BOHB_Bracket
...
BENCHMARKS = [
    ...
    ("rbv2_svm",     "16",      "trainsize", "acc"),
    ("rbv2_glmnet",  "16",      "trainsize", "acc"),
    ("rbv2_ranger",  "16",      "trainsize", "acc"),
    ("lcbench",      "3945",    "epoch",     "val_accuracy"),
]
...
def run(optimiser_class, scenario, instance, fidelity_param, metric,
    ...
    # enable diagnostics for BOHB variants
    if hasattr(optimiser, "set_diagnostic_prefix"):
        optimiser.set_diagnostic_prefix(
            f"{optimiser_class.__name__}_{scenario}_{instance}_seed{seed}"
        )
    ...
    for scenario, instance, fidelity_param, metric in BENCHMARKS:
        ...
        for optimiser_class in OPTIMISERS:
            for seed in range(N_SEEDS):
                fname = os.path.join(
                    RESULTS_DIR,
                    f"{optimiser_class.__name__}_{scenario}_{instance}_seed{seed}.pkl"
                )
